...
load('INPUT_TO_SILS_Multiple.RData')

cat('d:',d,' n:', n, ' b_n:', MultiBns,'\n')
...
U_mat = matrix(0, p, length(MultiBns))
Gamma_hat_B_mat = matrix(0, p, length(MultiBns))
G_hat_mat = array(0, dim=c(p, n_S1, length(MultiBns)))
G_breve_mat = matrix(0, p, length(MultiBns))
	
	
for(bn_ind in 1:length(MultiBns)){
	  b_n = MultiBns[bn_ind]
	  N = MultiNs[bn_ind]
	  JK_size = MultiJKs[bn_ind]
  ...
  #	gamma_hat_B = gamma_hat_B/N_hat - U^2
  	U_mat[, bn_ind] = U_gamma_N[1,]/U_gamma_N[3,]
	  Gamma_hat_B_mat[, bn_ind] =  U_gamma_N[2,]/U_gamma_N[3,] - U_mat[, bn_ind]^2
	###############################################################################
  ...
  #for(idx in 1:n_S1){#algorithm_two modified
	g_hat = foreach(idx = 1:n_S1, .combine=cbind, .packages=c('Matrix','Rfast')) %dopar% {
      ...
      for(qi in query_index){
        ...
        tmp_max = vector_idx
        tmp_min = vector_idx
        tmp_weight = rep(tmplocalWeightMat[idx], JK_T_1)*(b_n^(d/2))	
    ...
    oneColRes
	  ...
	  G_hat_mat[ , , bn_ind] = g_hat
  	G_breve_mat[ ,bn_ind] = rowMeans(g_hat)
  ...
  ### Bootstrap 
	U_max_without_normalization = sqrt(n)*max(U_mat)
	cnt_gt_times = 0
	for(b in 1:B){
		U_B = sqrt(Gamma_hat_B_mat) * matrix(rnorm(p*length(MultiBns)), p, length(MultiBns))
		
		# Bootstrap \gamma_A
		xi_prime = rnorm(n_S1)
		U_A = matrix(0, p, length(MultiBns))
		for(id_bn in 1:length(MultiBns)){
  		U_A[ ,id_bn] =  G_hat_mat[ , , id_bn] %*% matrix(xi_prime, ncol = 1) -sum(xi_prime)*G_breve_mat[ ,id_bn]
	  	U_A[ ,id_bn] = as.matrix( U_A[ ,id_bn] / sqrt(n_S1))
		}
		# Combining U_A and U_B to get the bootstrapped U-statistic
		Uxi = r* U_A +   U_B %*% diag(sqrt(n/MultiNs))
