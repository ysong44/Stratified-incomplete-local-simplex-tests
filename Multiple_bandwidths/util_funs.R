...
reg_fun_3 = function(X, paras) #two centers
{
  d = ncol(X)
  n = nrow(X)
  res_pos = rep(paras$coeff[1],n)
  res_neg = rep(paras$coeff[2],n)
  for(id in 1:d){
	res_pos = res_pos *  exp(-(X[,id] - paras$mu[id,1])^2/2/paras$ssq[1])
        res_neg = res_neg *  exp(-(X[,id] - paras$mu[id,2])^2/2/paras$ssq[2])
  }
...
reg_fun_meta = function(X, reg_opt, alpha)
{
  ...
  if(3==reg_opt){ mf = reg_fun_3(X,alpha)
  }
  return(mf)
}
